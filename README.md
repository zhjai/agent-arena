...
- When running inside **Codex**, invite **Claude Code** by default **if available and allowed**. If shell access exists, Codex should check `command -v claude && claude --version` before falling back to same-model subagents; the external `claude` CLI counts as the heterogeneous counterpart even if it is not exposed as a built-in Codex agent tool. Context minimization should not block useful review: allow Claude Code to read relevant source/docs/tests within the approved repo scope, while excluding secrets, datasets, generated results, private logs, and unrelated directories unless explicitly approved. In Claude Code print mode, size `--max-turns` for expected file/tool exploration. `error_max_turns` before any answer is a mechanical orchestration failure, not a substantive result: first check whether you boxed an **open design/architecture review** (which needs broad read-only tools + ample turns) as bounded verification — that misclassification, not the turn number, is the usual cause. Retry **once with lossless moves only** (resume session, raise the cap, add an explicit turn-budget contract, or drop discovery tools while keeping `Read`); **stop and ask the user before any lossy move** that narrows what the reviewer can independently examine (disabling `Read` for orchestrator-picked excerpts, narrowing scope, or accepting a no-critique result) — limiting the reviewer's tools to force a pass is the very failure arena exists to prevent. For non-trivial arenas, run multi-round critique/revision rather than a single one-shot call. If the user asks to design/build something together, use `collaborative_design` and make Claude Code a co-designer/architecture partner, not merely a reviewer.
